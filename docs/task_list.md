...
- [ ] **New `/players` fields (Session 83 audit)** — `/contract` and the stat endpoints are fully covered, but `/players` has documented fields we don't store (added April/July 2026). Priority: (1) `Organization ID` — reliable org attribution when OOTP leaves Parent Team ID unset for MLB teams; (2) `?retired=0` refresh filter — cheap refresh-speed win (skip retired players); (3) `League ID` on the player row is **negative for international-complex players** — clean intl-player identification; (4) `bats`/`throws` now on `/players` (authoritative handedness source vs ratings); (5) `years_protected_from_rule_5` / `draft_eligible` for roster/protection tooling. Lower value: `secondary_service_*`, `last_team_id`, `hall_of_fame`/`inducted`, `draft_supplemental`. Additive schema columns + `_upsert_players` reads (same shape as Phase 1, Session 69). See `docs/statsplus_api_analysis.md` §7. **LOE: Low-Medium.**
