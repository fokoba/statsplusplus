...
- [ ] **Per-tool, per-league transform curves (IN PROGRESS)** — Replace the single hardcoded global `tool_transform` (flat 1.3× above 60 / 1.5× below 40) with per-tool, per-league curves derived from each league's marginal-WAR-by-rating-band data during calibration. Marginal WAR is strongly **convex** at the top of the rating distribution and the convexity **varies by tool** (Session 82 eMLB data: contact 55-65 +2.69 → 65+ +6.25; power/eye/stuff similar; gap near-linear; speed nearly flat; RP tools mostly flat/noisy). The flat global curve under-rewards standout skills (a 100-stuff ace can't separate from movement-heavy arms). Design: piecewise-linear curve per tool anchored at rating bands, fit from **residualized** marginal WAR (remove correlated tools' contribution to avoid a tool inheriting others' signal — cf. the stuff-vs-movement weight problem), **shrunk toward a per-tool prior** by sample size (thin top bands and RP fall back to prior), normalized so 50→50, stored in `model_weights.json` as `TOOL_TRANSFORMS` and loaded by a tool-aware `tool_transform`. Touches calibration, the core transform, both composite functions, storage/migration, and tests. **LOE: Medium-High.**
- [ ] **Pitcher weight calibration — validate after next recalibration** — Session 82 dropped arsenal from the regression and added prior shrinkage (`shrink_weights_toward_prior`), but these only take effect on the next `calibrate.py` run and the ordering wasn't fully resolved by weights alone (Crochet still graded low in a dry run). Re-run calibration once the per-tool transform work lands, then confirm the elite-arm ordering (McClanahan/Crochet/Rohnson vs fringe RPs) resolves. The transform convexity is expected to do most of the remaining work. Consider whether `derive_tool_weights` should move from per-feature r² to multivariate OLS (Session 82 analysis: OLS gives RP stuff β=0.30 vs r²'s 0.08 — r² starves correlated causal tools). **LOE: Low-Medium.**
