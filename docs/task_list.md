...
- [ ] **Codebase quality review & refactor** — Systematic audit and restructure into proper Python package (`src/statsplusplus/`). Phases 1-6 complete: typed models, pure evaluation layer, config/client/data packages, web request-scoped context (17→1 connections per page), Flask blueprints, CLI entry points. Remaining: complete web route migration to blueprints, split query modules, delete legacy code, update all documentation. See `docs/refactoring_plan.md` for detailed status. **LOE: High (iterative, multi-session). In Progress.**
