...
- [ ] **Composite function consolidation** — `compute_composite_hitter` and `compute_composite_pitcher` exist in TWO places: `evaluation/composite.py` (pure package) and `data/evaluation_engine.py` (pipeline). They must stay in sync or bugs appear. Determine which is canonical, make the other a re-export or thin wrapper. If both are needed (different signatures/contexts), give them distinct names. Currently they have identical implementations that must be manually kept in sync. **LOE: Low.**
- [ ] **Model parameter calibration from data** — Several parameters in the player value model and imbalance penalties were set by reasoning rather than derived from data: (1) stat_confidence exponent (1.4), (2) option value multiplier (0.30 × gap × youth), (3) hitter imbalance penalty rates (0.15 spread, 0.12 weakness), (4) pitcher imbalance penalty rates (0.20 spread, 0.15 weakness), (5) RP surplus discount scale (0.35-0.65), (6) near-maxed blend threshold (0.7). Need: run composite scores against actual WAR production to validate penalty magnitudes. Compare extreme-profile players' composite scores to their actual WAR production — do penalized players actually underperform? Calibrate rates from the residuals. **LOE: Medium.**
...
- [ ] **Unified evaluation model** — Replace dual-system (prospect FV + MLB contract surplus) with single evaluation pipeline that smoothly transitions from tool-based to stat-based projection via `stat_confidence` ramp. Eliminates the hard cutoff at level=1, fixes spring training invites, first-callup noise, and crossover discontinuities. Design: `docs/unified_evaluation_design.md`. Implementation plan: `docs/unified_evaluation_implementation.md`. **Phases 1-4 complete. Phase 3 consumer wiring done via legacy table overwrite. Remaining: display-layer cleanup (replace `contract_value.py` year-by-year breakdown and `prospect_value.py` option-value display with unified model equivalents, enabling full deletion of legacy scripts).**
