...
- [x] **Model parameter calibration from data** — All tuning parameters extracted to `constants.py` with per-league override via `ModelWeights.get_param()`. Regression testing script (`scripts/model_regression.py`) validates composite vs WAR and calibrates aging curves + imbalance thresholds per-league. Defense weights recalibrated (SS/2B 0.05→0.15, 3B 0→0.10). Speed×contact synergy bonus added. Position-specific stat blend reduction for defense-first positions. EMLB hitter R² improved from 0.606→0.680 (63% of gap to OVR closed). **Done Session 79. Remaining: integrate aging/imbalance calibration into `calibrate.py` pipeline, weak-side contact bonus (r=0.12 signal identified but not yet implemented), further R² improvement on VMLB.**
