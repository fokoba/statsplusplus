...
def build_fixture(slug: str, with_ovr: bool) -> Path:
    ...
    # player_evaluation feeds the prospect_fv / player_surplus views.
    conn.execute(
        "INSERT OR REPLACE INTO player_evaluation "
        "(player_id, eval_date, name, bucket, age, composite, ceiling, fv, fv_str, "
        " fv_continuous, surplus, surplus_yr1, level, team_id, parent_team_id, "
        " stat_confidence, risk) VALUES "
        "(103, ?, 'Bob Prospect', 'SS', 21, 50, 62, 50, '50', 50.0, 8000000, 0, "
        " 'AA', ?, ?, 0.1, 'Medium')",
        (EVAL_DATE, TEAM_ID, TEAM_ID),
    )
    for pid, name, bkt, comp in ((100, "Joe Hitter", "CF", 56), (101, "Sam Starter", "SP", 59)):
        conn.execute(
            "INSERT OR REPLACE INTO player_evaluation "
            "(player_id, eval_date, name, bucket, age, composite, ceiling, fv, fv_str, "
            " surplus, surplus_yr1, level, team_id, parent_team_id, stat_confidence) VALUES "
            "(?, ?, ?, ?, 27, ?, 62, 55, '55', 12000000, 4000000, 'MLB', ?, 0, 0.9)",
