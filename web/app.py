...
def api_player_percentiles(pid):
    """Return percentile rankings for a specific year and optionally a specific league."""
    ...
    league_id = request.args.get("league_id", type=int)  # None = MLB, int = MiLB league
    if not year:
    ...
    if is_pitcher:
        data = get_pitcher_percentiles(pid, split_id=split_id, year=year, league_id=league_id)
    else:
        data = get_hitter_percentiles(pid, split_id=split_id, year=year, league_id=league_id)
    if not data:
        return jsonify({"error": "no data for year"}), 404
    return jsonify({"year": year, "stats": data, "league_id": league_id})
